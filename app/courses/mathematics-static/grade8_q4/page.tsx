...
export default function Lesson_grade8_q4() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-accent/5">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Функции и графики</h1>
          <p className="text-muted-foreground">Математика • 8 класс • 4 четверть</p>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border/60 p-8 mb-8">
          <div className="prose prose-lg max-w-none">
            ...
            <p className="my-2">Линейная функция — y=kx+m, её график — прямая; k — угловой коэффициент (наклон), m — точка пересечения с осью Oy. Квадратичная функция — y=ax²+bx+c (a≠0), её график — парабола с вершиной в точке (-b/2a; f(-b/2a)); ветви направлены вверх при a{'>'}0 и вниз при a{'<'}0.</p>
            <h2 className="text-2xl font-bold mt-6 mb-4">Ключевые формулы</h2>
            <p className="my-2">Линейная функция: y=kx+m; k{'>'}0 — возрастает, k{'<'}0 — убывает</p>
            <p className="my-2">Вершина параболы: x₀ = -b/(2a), y₀ = f(x₀)</p>
            <p className="my-2">Ось симметрии параболы: x = -b/(2a)</p>
            <p className="my-2">Нули функции — корни уравнения ax²+bx+c=0</p>
            <h2 className="text-2xl font-bold mt-6 mb-4">Примеры решения задач</h2>
            <p className="font-bold mt-4">Пример 1. Найдите точку пересечения графика y=2x-3 с осью Ox.</p>
            <p className="ml-4 text-muted-foreground">Решение: y=0: 2x-3=0, x=1,5. Точка (1,5; 0).</p>
            <p className="font-bold mt-4">Пример 2. Найдите вершину параболы y=x²-4x+3.</p>
            <p className="ml-4 text-muted-foreground">Решение: x₀=-(-4)/(2·1)=2. y₀=4-8+3=-1. Вершина (2;-1).</p>
            <p className="font-bold mt-4">Пример 3. Постройте эскиз графика y=-x²+2x. Куда направлены ветви?</p>
            <p className="ml-4 text-muted-foreground">Решение: a=-1{'<'}0 — ветви вниз. Вершина x₀=1, y₀=-1+2=1. Вершина (1;1).</p>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border/60 p-8">
          ...
          <div className="aspect-video">
            <iframe
              ...
              src="https://www.youtube.com/embed/mVcDGuw4hv8"
              title="YouTube video"
        ...
        <div className="bg-card rounded-xl border border-border/60 p-8">
          ...
          <Quiz questions={[
            { question: "Решите неравенство: x+3>5", options: ["x>2", "x<2", "x≥2", "x=2"], correct: 0 },
            { question: "Решите: 2x≤8", options: ["x≤4", "x<4", "x≥4", "x=4"], correct: 0 },
            { question: "При каких x: x²<4?", options: ["-2<x<2", "x<2", "x>-2", "x≠±2"], correct: 0 },
            { question: "Решите: -x>3", options: ["x<-3", "x>-3", "x<3", "x>3"], correct: 0 },
            { question: "Решите систему: x>1, x<5", options: ["1<x<5", "x>1", "x<5", "нет решений"], correct: 0 },
            { question: "При каких x: |x|≤3?", options: ["-3≤x≤3", "x≤3", "x≥-3", "x=±3"], correct: 0 },
            { question: "Решите: 3x-6≥0", options: ["x≥2", "x>2", "x≤2", "x=2"], correct: 0 },
            { question: "Чему равно решение x-5<0?", options: ["x<5", "x>5", "x≤5", "x≠5"], correct: 0 },
            { question: "Решите: x/2>1", options: ["x>2", "x<2", "x>1/2", "x>0"], correct: 0 },
            { question: "При каких x: x²≥0?", options: ["любое x∈R", "x≥0", "x>0", "нет решений"], correct: 0 }
          ]} />
        </div>
