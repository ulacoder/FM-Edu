  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-accent/5">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Показательная функция</h1>
          <p className="text-muted-foreground">Математика • 11 класс • 2 четверть</p>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border/60 p-8 mb-8">
          <div className="prose prose-lg max-w-none">
            ...
            <p className="my-2">Показательная функция y=aˣ, где a{'>'}0, a≠1. При a{'>'}1 функция возрастает, при 0{'<'}a{'<'}1 — убывает. Область определения — все действительные числа, область значений — (0;+∞). Показательное уравнение вида aᶠ⁽ˣ⁾=aᵍ⁽ˣ⁾ равносильно f(x)=g(x).</p>
            <h2 className="text-2xl font-bold mt-6 mb-4">Ключевые формулы</h2>
            <p className="my-2">aˣ·aʸ = aˣ⁺ʸ; aˣ/aʸ = aˣ⁻ʸ; (aˣ)ʸ = aˣʸ</p>
            <p className="my-2">aˣ = aʸ ⟺ x = y (при a{'>'}0, a≠1)</p>
            <p className="my-2">D(aˣ) = R, E(aˣ) = (0;+∞)</p>
            <p className="my-2">a⁰=1 для любого a{'>'}0</p>
            <h2 className="text-2xl font-bold mt-6 mb-4">Примеры решения задач</h2>
            <p className="font-bold mt-4">Пример 1. Решите уравнение 2ˣ = 8.</p>
            <p className="ml-4 text-muted-foreground">Решение: 8=2³, значит 2ˣ=2³ → x=3.</p>
            <p className="font-bold mt-4">Пример 2. Решите 3²ˣ⁻¹ = 27.</p>
            <p className="ml-4 text-muted-foreground">Решение: 27=3³, 2x-1=3, x=2.</p>
            <p className="font-bold mt-4">Пример 3. Сравните 2³ и 2⁵ по возрастанию функции.</p>
            <p className="ml-4 text-muted-foreground">Решение: Основание 2{'>'}1, функция возрастает, значит 2³{'<'}2⁵.</p>
          </div>
  
        <div className="bg-card rounded-xl border border-border/60 p-8">
          <h2 className="text-2xl font-bold mb-6">Тест на понимание 📝</h2>
          <Quiz questions={[
            { question: "Чему равен lim(x→0) sinx/x?", options: ["1", "0", "∞", "не существует"], correct: 0 },
            { question: "Найдите lim(x→∞) (1/x)", options: ["0", "1", "∞", "не существует"], correct: 0 },
            { question: "Вычислите: lim(x→2) x²", options: ["4", "2", "0", "∞"], correct: 0 },
            { question: "Чему равен lim(x→0) (x²/x)?", options: ["0", "1", "∞", "не определено"], correct: 0 },
            { question: "Найдите lim(x→∞) (3x+1)/x", options: ["3", "1", "∞", "0"], correct: 0 },
            { question: "Чему равна производная в точке разрыва?", options: ["Не существует", "0", "∞", "Зависит"], correct: 0 },
            { question: "Вычислите: lim(x→1) (x-1)/(x²-1)", options: ["1/2", "1", "0", "не существует"], correct: 0 },
            { question: "Чему равен lim(x→0⁺) (1/x)?", options: ["+∞", "-∞", "0", "1"], correct: 0 },
            { question: "Непрерывна ли f(x)=|x| в x=0?", options: ["Да", "Нет", "Зависит", "Не определено"], correct: 0 },
            { question: "Найдите lim(x→∞) e⁻ˣ", options: ["0", "∞", "e", "1"], correct: 0 }
          ]} />
        </div>
